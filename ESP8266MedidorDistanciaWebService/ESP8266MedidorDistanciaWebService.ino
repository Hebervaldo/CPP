//****************************************************************************************
//                             ESP 8266 Medidor de Distancia 
//                       Hebervaldo de Paula Carvalhedo, Julho 2017
//****************************************************************************************

// Build 1
//   r1 - medidor de distancia, webserver, email, etc.
...
#define PINO_TRIGGER PINO_GPIO4
#define PINO_ECHO PINO_GPIO5
...
bool blnPrimeiroMedidaUltraSom = false;
...
String assunto = "ESP 8266 - Medidor de distancia";
String mensagem = "";
...
void mtdExecutarAferirDistanciaUltraSom()
{
  pinMode(PINO_GPIO12, OUTPUT);
  digitalWrite(PINO_GPIO12, HIGH);
  ...
  pinMode(PINO_GPIO12, OUTPUT);
  digitalWrite(PINO_GPIO12, LOW);

  blnPrimeiroMedidaUltraSom = true;

  // if(modoLedIndicador)
  // {
  //   if((((int) distancia_cm) % 2) == 0)
  //   {
...
void mtdGerarPaginaWebService()
{
  ...
  // buf += "<p>Permitir que o led azul interaja com a distancia lida pelo sensor ultrassonico. <a href=\"?function=btnLedIndicador_on\"><button>ON</button></a> <a href=\"?function=btnLedIndicador_off\"><button>OFF</button></a></p>";
  ...
  buf += "<br>";
  buf += "Distancia (cm): ";
  buf += String(distancia_cm);
  buf += "<br>";
  buf += "Distancia (pol): ";
  buf += String(distancia_in);
  buf += "<br>";
  buf += "Numero boot: ";
  buf += String(boot_num);
  buf += "<br>";
  ...
  buf += "<br>";
  buf += "Senha: ";
  buf += String(rbase64.encode(strSenha));
  // buf += String(strSenha);
  buf += "<br>";
  buf += "Servidor: ";
  ...
  // if(req.indexOf("btnLedIndicador_on") != -1)
  // {
  //   modoLedIndicador = true;
  // }

  // if(req.indexOf("btnLedIndicador_off") != -1)
  // {
  //   modoLedIndicador = false;
  // }
...
byte mtdEnviarEmail(String Emitente, String Recebedor, String Usuario, String Senha, String Servidor, int Porta, String Assunto, String Mensagem)
{
  ...
  String Usuario_base64 = rbase64.encode(Usuario);
  String Senha_base64 = rbase64.encode(Senha);
...
{
  assunto = "ESP 8266 - Medidor de distancia";
  ...
  mensagem += uptime;
  mensagem += "\n\r";
  mensagem += "Distancia (cm): ";
  mensagem += String(distancia_cm);
  mensagem += "\n\r";
  mensagem += "Distancia (pol): ";
  mensagem += String(distancia_in);
  mensagem += "\n\r";
  ...
  mensagem += "\n\r";
  mensagem += "Senha: ";
  mensagem += String(rbase64.encode(strSenha));
...
void mtdEscreverEEPROM(String Entrada, int Inicio, int Fim)
{
  ...
  {
    Serial.print(F("O comprimento da String deve ser menhor do que a variacao entre Inicio e Fim: ")); 
...
{
  pinMode(PINO_GPIO0, OUTPUT);
  digitalWrite(PINO_GPIO0, HIGH);
  pinMode(PINO_GPIO0, INPUT);
  pinMode(PINO_GPIO2, OUTPUT);
  digitalWrite(PINO_GPIO2, HIGH);
  pinMode(PINO_GPIO2, INPUT);
  pinMode(PINO_GPIO4, OUTPUT);
  digitalWrite(PINO_GPIO4, HIGH);
  pinMode(PINO_GPIO4, INPUT);
  pinMode(PINO_GPIO15, OUTPUT);
  digitalWrite(PINO_GPIO15, LOW);
  pinMode(PINO_GPIO12, OUTPUT);
  digitalWrite(PINO_GPIO12, LOW);
  // pinMode(PINO_GPIO12, INPUT);
  pinMode(PINO_GPIO13, OUTPUT);
  digitalWrite(PINO_GPIO13, HIGH);
  pinMode(PINO_GPIO13, INPUT);
  pinMode(PINO_GPIO16, OUTPUT);
  digitalWrite(PINO_GPIO16, HIGH);
}
...
void setup()
{
  ...
  blnPrimeiroEmail = true;
  blnPrimeiroMedidaUltraSom = false;
...
void loop()
{
  if(cycleCheck(&thread1LastMillis, CycleThread1))
  {
    ...
    mtdExecutarAferirDistanciaUltraSom();
