...
#include "NOKIA_5110_Print.hpp"  //custom
...
public:
	...
	void drawChar(uint16_t x, uint16_t y, uint8_t c, uint8_t color, uint8_t bg, uint8_t size);
	...
	void drawBitmap(uint16_t x, uint16_t y, const uint8_t *bitmap, uint16_t w, uint16_t h, uint8_t color);
	void customChar(const uint8_t *c);
